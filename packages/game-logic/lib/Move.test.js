...
describe('Move', () => {
    ...
    describe('basic validation', () => {
        ...
        it('validates destination can accept the piece', () => {
            ...
            const move = new Move(game.player1, game.player1.pool.stacks[0], // Has pieces from size 0-3, top is size 3
    ...
    describe('pool to board moves', () => {
        ...
        it('blocks moves from pool to occupied spaces (without three-in-a-row)', () => {
            ...
            const move = new Move(game.player1, game.player1.pool.stacks[1], // Different stack
        ...
        it('allows covering opponent pieces when they have three in a row', () => {
            ...
            const move = new Move(game.player1, game.player1.pool.stacks[1], game.board.stacks[1][2] // Cover the third piece in player2's row
            );
    ...
    describe('board to board moves', () => {
        ...
        it('allows board to board moves to occupied spaces (if size allows)', () => {
            ...
            const move = new Move(game.player1, game.board.stacks[0][0], // size 3 piece
        ...
        it('prevents board to board moves to spaces with larger pieces', () => {
            ...
            const move = new Move(game.player1, game.board.stacks[0][1], // size 2 piece
    ...
    describe('three-in-a-row detection', () => {
        it('detects horizontal three-in-a-row', () => {
            ...
            game = Game.deserialize(moves);
            const move = new Move(game.player2, game.player2.pool.stacks[1], game.board.stacks[0][1] // Cover middle of player1's three
            );
            expect(move.coversOneOfThree()).toBe(true);
        });
        it('detects vertical three-in-a-row', () => {
            ...
            game = Game.deserialize(moves);
            const move = new Move(game.player2, game.player2.pool.stacks[1], game.board.stacks[1][0] // Cover middle of player1's three
            );
            expect(move.coversOneOfThree()).toBe(true);
        });
        it('detects diagonal three-in-a-row', () => {
            ...
            game = Game.deserialize(moves);
            const move = new Move(game.player2, game.player2.pool.stacks[1], game.board.stacks[1][1] // Cover middle of player1's diagonal
            );
            expect(move.coversOneOfThree()).toBe(true);
        });
        it('does not detect two-in-a-row as three-in-a-row', () => {
            ...
            game = Game.deserialize(moves);
            const move = new Move(game.player1, game.player1.pool.stacks[1], game.board.stacks[1][0] // Try to cover player2's piece
            );
