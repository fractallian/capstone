...
`);
    ...
    describe('move from pool to cover piece exception', () => {
        ...
        it('allows the exception', () => {
            gameWithThreeInRow.makeMove(gameWithThreeInRow.player1.pool.stacks[1], gameWithThreeInRow.board.stacks[1][2]);
      ...
      `);
        ...
        it('does not allow otherwise', () => {
            gameWithThreeInRow.makeMove(gameWithThreeInRow.player1.pool.stacks[0], gameWithThreeInRow.board.stacks[2][0]);
            expect(() => gameWithThreeInRow.makeMove(gameWithThreeInRow.player2.pool.stacks[2], gameWithThreeInRow.board.stacks[2][0])).toThrowError(InvalidMoveError);
            expect(viewGameState(gameWithThreeInRow)).toMatchInlineSnapshot(`
        "
          ||X3|X2|X1|  ||O0
        X3||O3|O2|O1|  ||O3
      ...
      `);
        ...
        it('allows pool to empty space even with three in row', () => {
            ...
            expect(() => {
                gameWithThreeInRow.makeMove(gameWithThreeInRow.player1.pool.stacks[1], gameWithThreeInRow.board.stacks[3][3]);
            }).not.toThrow();
        });
    ...
    describe('edge cases', () => {
        ...
        it('handles maximum stacking scenario', () => {
            ...
            expect((_a = targetStack.topPiece()) === null || _a === void 0 ? void 0 : _a.size).toBe(PieceSize.Two); // Size 1 piece on top
        });
