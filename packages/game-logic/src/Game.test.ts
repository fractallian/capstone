...
`);
	...
	describe("cloning", () => {
		...
		it("can continue playing from clone", () => {
			...
			// Make move in clone
			clonedGame.makeMove(
				clonedGame.player2.pool.stacks[0],
				clonedGame.board.stacks[1][1],
			);
	...
	describe("move from pool to cover piece exception", () => {
      ...
      `);
		...
		it("prevents pool to occupied space without three in row exception", () => {
			...
			simpleGame.makeMove(
				simpleGame.player1.pool.stacks[0],
				simpleGame.board.stacks[0][0],
			);
			simpleGame.makeMove(
				simpleGame.player2.pool.stacks[0],
				simpleGame.board.stacks[1][0],
			);

			// Try to cover opponent's piece from pool
			expect(() => {
				simpleGame.makeMove(
					simpleGame.player1.pool.stacks[1],
					simpleGame.board.stacks[1][0],
				);
			}).toThrow(InvalidMoveError);
		});
